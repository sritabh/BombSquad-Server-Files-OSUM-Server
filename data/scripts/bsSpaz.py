...
class SpazFactory(object):
    ...
    def __init__(self):
        ...
        self.spazMaterial.addActions(
            ...
            actions=(('message', 'ourNode', 'atConnect', 'footing', 1),
                     ('message', 'ourNode', 'atDisconnect', 'footing', -1)))
                        #will update b4 anyone notice this
...
class Spaz(bs.Actor):
    ...
    def onJumpPress(self):
        ...
        if t - self.lastJumpTime >= self._jumpCooldown:
            ...
            if bs.getGameTime() - self.lastPunchTime<=300 and self.node.jumpPressed and self.node.punchPressed and bs.getGameTime() - self.lastBackflipTime >= self._backflipCooldown:
            	if not self._kick:
            	    factory = self.getFactory()
                    self._kick = True
                    #add kick material
    ...
    def handleMessage(self, msg):
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            if (msg.powerupType == 'tripleBombs'):
                ...
                bsUtils.PopupText("GlueBomb",color=(0,1,0),scale=1.5,position=self.node.position).autoRetain()
            	...
            	def remove():
            	     self.node.color = self._color
                bs.gameTimer(20000,bs.Call(remove))
            	tex = bs.Powerup.getFactory().texStar
            	self._punchCooldown=0
                self._punchPowerScale = 1.0
                bsUtils.PopupText("Punch Repeatedly!",color=(0.4,0.3,1),scale=1.6,position=self.node.position).autoRetain()
                ...
                if self.powerupsExpire:
                    ...
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._starWearOffFlashTimer = bs.Timer(gPowerupWearOffTime-1000,bs.WeakCall(self._starWearOffFlash))
                    self._starWearOffTimer = bs.Timer(gPowerupWearOffTime,bs.WeakCall(self._starWearOff))
            elif msg.powerupType == 'invisibility':
            	def name():
            	     self.node.name = self._name
                bs.gameTimer(20000,bs.Call(name))
                tex = bs.Powerup.getFactory().texInvisibility
                self._flashBillboard(tex)
                self.setInvisibility()
                self._isInvisible = True
                bsUtils.PopupText("And His Name is \nJOHN CENA!",color=(1,1,0),scale=1.2,position=self.node.position).autoRetain()
                if self.powerupsExpire:
                    self.node.miniBillboard3Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard3StartTime = t
            ...
            if msg.hitType == 'kick':
                ...
                if damage > 99999:
                    bsUtils.PopupText("#OSUM!",color=(1,0.5,0),scale=1.6,position=self.node.position).autoRetain()
                    self.lightningBolt(position = self.node.position,radius = 3)

                if damage > 800 and damage < 99999:
                    bsUtils.PopupText("#OSUM!",color=(0,1,0.3),scale=1.6,position=self.node.position).autoRetain()
            ...
            if msg.hitType == 'punch':
                ...
                elif damage > 900 and damage < 99999:
                    bsUtils.PopupText("#OSUM!",color=(0,1,0.3),scale=1.6,position=self.node.position).autoRetain()
            ...
            self.curseExplode()
        elif isinstance(msg, _KickHitMessage):
            ...
            if (node is not None and node.exists()
                ...
                kickMomentumAngular = ((self.node.punchMomentumAngular or self.node.run) * 0.77)
                ...
                kickDir = (self.node.velocity[0],self.node.velocity[1]+5,self.node.velocity[2])
    ...
    def _starWearOffFlash(self):
        if self.node.exists():
            self.node.billboardTexture = bs.Powerup.getFactory().texStar
            self.node.billboardOpacity = 1.0
            self.node.billboardCrossOut = True
            
    def _starWearOff(self):
        factory = self.getFactory()
        self._punchPowerScale = factory.punchPowerScale
        self._punchCooldown = factory.punchCooldown
        self.node.color = self._color
