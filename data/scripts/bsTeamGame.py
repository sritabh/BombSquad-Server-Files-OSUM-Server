...
gDefaultTeamColors = ((1,1,1), (0.4,0.2,0.1))
gDefaultTeamNames = u'\ue049Gods', u'\ue046Devils'
...
class TeamsScoreScreenActivity(bsGame.ScoreScreenActivity):
    ...
    def showPlayerScores(
        ...
        for p in playersSorted:
            ...
            topBetrayCount = max(topBetrayCount, p.accumBetrayCount)
    ...
    def onBegin(self):
        ...
        if mvp is not None:
            bsUtils.Text(bs.Lstr(resource='mostViolentPlayerText'),
                         ...
                         vAlign='center',
                         maxWidth=300,
                         position=(180, tsHeight/2-100+vExtra+15),
                         transition='inLeft',
                         hAlign='left', transitionDelay=t).autoRetain()
            bsUtils.Text(
                bs.Lstr(
                ...
                position=(260, tsHeight / 2 - 100 - 15 + vExtra),
            ...
            bsUtils.Image(
                mvp.getIcon(),
                position=(233, tsHeight / 2 - 100 - 30 - 46 + 25 + vExtra),
                scale=(50, 50),
                transition='inLeft', transitionDelay=t).autoRetain()
            bsUtils.Text(
                bs.Lstr(value=mvpName),
                position=(270, tsHeight / 2 - 100 - 30 - 36 + vExtra + 15),
        ...
        if mkp is not None:
            bsUtils.Text(bs.Lstr(resource='mostViolatedPlayerText'),
                         ...
                         vAlign='center',
                         maxWidth=300,
                         position=(180, tsHeight/2-250+vExtra+15),
                         transition='inLeft',
                         hAlign='left', transitionDelay=t).autoRetain()
            bsUtils.Text(
                bs.Lstr(
                ...
                position=(260, tsHeight / 2 - 250 - 15 + vExtra),
                hAlign='left', scale=0.6, color=(0.3, 0.3, 0.3, 1.0),
                transition='inLeft', transitionDelay=t).autoRetain()
            t += 4*tIncr
            bsUtils.Image(
                mkp.getIcon(),
                position=(233, tsHeight / 2 - 250 - 30 - 46 + 25 + vExtra),
                scale=(50, 50),
                transition='inLeft', transitionDelay=t).autoRetain()
            bsUtils.Text(bs.Lstr(value=mkpName),
                         position=(270, tsHeight/2-250-30-36+vExtra+15),
        ...
        # most suicide was lazy to change this just changed the values
        mostBetray = 0
        for p in playersSorted:
            if p[2].betrayCount >= mostBetray:
                mbp = p[2]
                mbpName = p[1]
                mostBetray = p[2].betrayCount
        if mbp is not None:
            bsUtils.Text("Mr.Betrayer",
                         color=(0.5, 0.5, 0.5, 1.0),
                         vAlign='center',
                         maxWidth=300,
                         position=(180, tsHeight/2-400+vExtra+15),
                         transition='inLeft',
                         hAlign='left', transitionDelay=t).autoRetain()
            bsUtils.Text("("+str(mostBetray)+" betrays)",
                position=(260, tsHeight / 2 - 400 - 15 + vExtra),
                color=(0.3, 0.3, 0.3, 1.0),
                scale=0.6, hAlign='left', transition='inLeft',
                transitionDelay=t).autoRetain()
            t += 4*tIncr
            ...
            bsUtils.Image(
                mbp.getIcon(),
                position=(233, tsHeight / 2 - 400 - 30 - 46 + 25 + vExtra),
                scale=(50, 50),
                transition='inLeft', transitionDelay=t).autoRetain()
            bsUtils.Text(
                bs.Lstr(value=mbpName),
                position=(270, tsHeight / 2 - 400 - 30 - 36 + vExtra + 15),
                hAlign='left', vAlign='center', maxWidth=180,
                color=bs.getSafeColor(mbp.getTeam().color + (1,)),
                transition='inLeft', transitionDelay=t).autoRetain()
            t += 4*tIncr
