        ...
        for i in roster:
            try:
                if i['players'][0]['nameFull'].lower().find(nick.encode('utf-8').lower()) != -1:
                    ...
                    f = open("/var/www/html/chats" + "/Chats " + date + ".html",'a')
                    f.write("<meta charset='UTF-8'><ul><li><b>Kicked: " + i['players'][0]['name'] + "(" + i['displayString'] + ")</b></li></ul>\n")
    ...
    def opt(self,nick,msg):
        if self.checkDevice(nick):
            ...
            with bs.Context(activity):
                if m == '/kick':
                    if a == []:
                        bsInternal._chatMessage('Using: /kick name or clientID')
                    else:
                        if len(a[0]) > 3:
                            self.kickByNick(a[0])
