...
class MagicBox(bs.Bomb):
	...
	def updateFloatyness(self):
		...
		newY = {0: 0, 1: 0, 2: 0 + 20 * 2, 3: 19 + 20 * 3}.get(self.heldBy, 0) # needs more science
	...
	def setScoreText(self, text):
		...
		self._scoreText.color = bs.getSafeColor((1,0.5,0))
...
class MagicBoxGame(bs.TeamGameActivity):
	...
	def getSettings(cls, sessionType):
		return [("Hold Time", {'minValue': 10, 'default': 60, 'increment': 10}),
	...
	def getInstanceDescription(self):
		return ('Hold the ball and start passing for ${ARG1} seconds.', self.settings['Hold Time'])

	def getInstanceScoreBoardDescription(self):
		return ('Keep The Ball for ${ARG1} Sec' , self.settings['Hold Time'])
